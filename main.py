...
BANKINFO = "BankData.txt"
TRANSINFO = "TransactionLog.txt"
...
def createAccountNumber(surname:str, id:str, name:str)->str:
    cut = id[slice(6, 10)]
    account = str(len(surname)) + cut + str(len(name))
    return account

def createAccount(name:str, surname:str, id:str, password:str):
    account = createAccountNumber(surname, id, name)
    entry = "\n"+account +","+name+","+surname+","+id+","+password
    global accountNr
    accountNr = account
    global path
    path = "TransactionLogs/"+accountNr+TRANSINFO
    try:
        bankDataFile = open(BANKINFO, "a+")
    except:
        bankDataFile = open(BANKINFO, "w")
    bankDataFile.write(entry)
    bankDataFile.close()
    return account
...
def validateLogIns(account:str, password:str):
    try:
        bankDataFile = open(BANKINFO, "r+")
        x = bankDataFile.readlines()
    except:
        bankDataFile = open(BANKINFO, "w")
        bankDataFile.close()
        return False
    ...
    if x:
        for line in x:
            b = line.split(",")
            if b[0] == account and b[4].strip() == password:
                bankDataFile.close()
                return True
        bankDataFile.close()
        return False
    ...
    try:
        ...
        if x:
            for line in x:
                ...
                if b[0] == accountNr:
                    balance = getBalance()
                    message = "==========================\n-----------------Balance----------------\n******************************\n\nName: " + b[1]+"\nSurname: " + b[2]+"\nAccount Number: " + b[0]+"\n\nCurrent Balance: R" + "{:.2f}".format(float(balance))+"\n\n******************************"
                    bankDataFile.close()
                    return message
    except:
        bankDataFile = open(BANKINFO, "w")
...
def getBalance():
    global path
    try:
        transInfoFile = open(path, "r+")
        x = transInfoFile.readlines()
        if x:
            for line in x:
                b = line
            c = b.split("R")
        ...
        transInfoFile = open(path, "w")
        transInfoFile.close()
        return "0"

def withdrawlAmount(amount:str):
    balance = getBalance()

    a = float(balance) - float(amount)
    global accountNr
    global path
    try:
        transFile = open(path, "a+")
    except:
        transFile = open(path, "w")
    transFile.write("\n=============================== \n" + accountNr + "\nTransaction Type: Withdrawal \nAmount Withdrawn: R" + "{:.2f}".format(float(amount)) + "\nPrevious Balance: R" + "{:.2f}".format(float(balance)) + "\nCurrent Balance : R" + "{:.2f}".format(a))
    transFile.close()
    print("===============================")
    print("Withdrawal Transaction Complete")
    print("*******************************")

def depositSet(amount:str):
    balance = getBalance()
    a = float(balance)+float(amount)
    global accountNr
    global path
    ...
    transFile.write("\n=============================== \n"+accountNr +"\nTransaction Type: Deposit \nAmount Deposited: R"+"{:.2f}".format(float(amount))+"\nPrevious Balance: R"+"{:.2f}".format(float(balance))+"\nCurrent Balance : R"+"{:.2f}".format(a))
    transFile.close()

#Navigation
def createPasswordGUI(name:str, surname:str, id:str):
    master = ctk.CTk()
    master.title("ZETA BANKING SERVICES")
    ...
    IMAGE_PATH = 'ZETA BANKING SERVICES.png'
...
def mainMenu():
    ...
    lblTitle = ctk.CTkLabel(master, text='ZETA BANKING SERVICES', font=('Arial Bold', 20))
